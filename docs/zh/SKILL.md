...
`draw-ui` 的核心任务是生成 UI 设计稿（Mockup），并协助将生成的 UI 截图还原为 HTML/CSS。

本 skill 不内置生图能力；生图/图生图由当前环境中的外部插件、MCP、其他 skill，或用户指定的工具提供。
...
当用户想把生成图、截图或设计稿还原成 HTML/CSS 时，先读取 `references/html-reconstruction.md`。核心原则：页面结构优先代码化；logo、品牌符号、复杂插画、3D/玻璃质感、半透明渐变等难复刻视觉元素要素材化。裁图只作为图生图参考和定位依据，最终放进 HTML 的复杂资产要用外部生图能力重绘，再裁边、抠图和清理边缘。
...
- **Prompt 控制在 800 字以内** — 超长会导致部分外部生图工具失败或截断
...
2. 当前环境只有一个可用生图工具时，直接使用该工具。
3. 当前环境有多个可用生图工具，且用户没有指定时，必须先让用户选择，不要自作主张选择。
4. 当前环境没有可用生图工具时，提示用户安装或启用外部插件、MCP 或 skill 后再继续。

调用外部生图能力时，把页面目标、业务上下文、真实示例数据、风格描述、参考图及其用途、目标比例或画布类型传给对应工具。

---

## Options

不同外部生图工具的参数名可能不同，按该工具支持的格式传递。建议语义如下：
...
成功出图后，记录图片路径或 URL，后续 HTML 还原和素材处理都基于该产物继续。
