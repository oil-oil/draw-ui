...
**局部裁图要作为图生图参考。** 对 logo、复杂卡片插图、hero 装饰图这类素材，先裁局部图，再把裁图交给当前环境中可用的外部生图能力（插件、MCP 或其他 skill），生成干净高分辨率素材。直接把裁图放进 HTML 容易出现糊、背景色不一致、边缘噪声和文字碎片。
...
不同外部生图工具对透明背景的支持不一致。需要透明素材时，如果所选工具不能稳定生成真实透明 PNG，优先生成纯白底或绿幕素材，再运行 skill 内置后处理脚本：
